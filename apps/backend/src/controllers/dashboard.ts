...
export const getDashboardStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const categoriesResult = await db.select({ count: count() }).from(categories)
    ...
    const completedOrdersResult = await db
      .select({ count: count() })
      .from(orders)
      .where(and(eq(orders.status, 'completed'), ...dateFilters))

    // Get pending orders count
    const pendingOrdersResult = await db
      .select({ count: count() })
      .from(orders)
      .where(and(eq(orders.status, 'pending'), ...dateFilters))
    ...
    orders.forEach((order) => {
      ...
      if (intervalValue === 'hour') {
        ...
        const daysSinceStart = Math.floor(
          (order.date.getTime() - startDate.getTime()) / (24 * 60 * 60 * 1000)
        )
