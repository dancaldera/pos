...
const MainLayout: React.FC = () => {
  ...
  const navigation = [
    ...
    {
      name: translate.products('title'),
      to: '/products',
      icon: SquaresPlusIcon,
      requiredRole: ['admin', 'manager'],
    },
    {
      name: translate.categories('title'),
      to: '/categories',
      ...
      <SidebarBody>
        <SidebarSection>
          {filteredNavigation.map((item) => (
            <SidebarItem key={item.name} href={item.to} current={location.pathname === item.to}>
      ...
      <SidebarFooter>
        <SidebarSection>
          <div className="flex items-center gap-3">
            ...
            <div>
              <Text>{user?.name}</Text>
              <Text className="text-xs text-zinc-500 dark:text-zinc-400 capitalize">
                {user?.role}
              </Text>
            </div>
          </div>
