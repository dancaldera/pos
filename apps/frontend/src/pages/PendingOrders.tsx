    ...
    <div className="min-h-screen">
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <Heading level={1}>{translate.orders('pendingOrders')}</Heading>
      ...
      {/* Items Modal */}
      <Dialog open={itemsModalOpen} onClose={closeItemsModal}>
        <DialogTitle>{translate.orders('items')}</DialogTitle>
        <DialogBody>
          {modalLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {orderItemsDetails.map((item) => (
                <div key={item.id} className="shadow rounded p-4 flex flex-col items-center">
                  {item.imageUrl ? (
                    <img
                      src={item.imageUrl}
                  ...
                  <Text className="text-sm font-semibold text-center">{item.productName}</Text>
        ...
        <DialogActions>
          <Button outline onClick={closeItemsModal}>
