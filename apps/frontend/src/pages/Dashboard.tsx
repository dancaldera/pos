...
const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsRes, recentOrdersRes, topProductsRes, paymentStatsRes, salesDataRes] =
        await Promise.all([
            ...
            <div className="flex justify-between items-center h-full">
              <div>
                ...
                <Text>
                  {isLoading
                    ? ''
                    : `${stats?.activeProducts || 0} ${translate.common('active').toLowerCase()}`}
                </Text>
              </div>
            ...
            <div className="flex justify-between items-center h-full">
              <div>
                ...
                <Text>
                  {isLoading
                    ? ''
                    : `${stats?.completedOrders || 0} ${translate.orders('completed').toLowerCase()}`}
                </Text>
              </div>
          ...
          <Card className="h-full">
            <div className="flex justify-between items-center h-full">
              <div>
                ...
                <Heading level={2}>
                  {isLoading || monthlyGrowth === null ? '...' : `${monthlyGrowth.toFixed(1)}%`}
                </Heading>
          ...
          <div className="h-80">
            {isLoading || salesData.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <Text>{isLoading ? translate.common('loading') : translate.reports('noData')}</Text>
              </div>
            ) : (
